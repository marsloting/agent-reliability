...
| agent 说 | 用这个 | 它强制什么 |
|---|---|---|
| "done / fixed / sent / scheduled / shipped" | [`agent-reliability-guardrails`](skills/agent-reliability-guardrails/) | 从真正会暴露错误的地方拿证据 |
| "我已经交给另一个 agent 了" | [`agent-to-agent-wake-protocol`](skills/agent-to-agent-wake-protocol/) | 接收方必须回显它实际收到的交接行 |
| "owner 还在跑 / 我要不要接管" | [`agent-supervisor-guardrails`](skills/agent-supervisor-guardrails/) | 核 owner、核产物、核 ledger，再判断等 / 提醒 / 接管 / 停手 |
| "批量都过了" | [首件检验](mechanisms/first-article-inspection.zh.md) | 第一件验透，再放批 |
| "我记得契约" | [跨域重读](mechanisms/cross-domain-re-read.zh.md) | 跨域边界必须重读当前源 |
...
| [复合衰减](mechanisms/compound-decay.zh.md) | 单步 85% 可靠，8 步只剩 27% | 先数链长；每 3-4 步插验证检查点；检查点 = 回滚单元 |
| [跨域重读](mechanisms/cross-domain-re-read.zh.md) | 记忆过期零报错 | 记忆是已过期缓存，跨域边界必重读契约 |
